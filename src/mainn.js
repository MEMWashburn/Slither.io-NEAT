...
function fitness(genome) {
  ...
  for (var i = 0; i < genome.scores.length; i++) {
    ...
    avgScore += genome.scores[i];
  }
  avgScore /= genome.scores.length;
  fitScore /= genome.scores.length;
  temp = avgScore / 10;
  while (temp > 10) {
    temp /= 10;
    fitScore *= 10
  }
  return avgScore + fitScore;
};
